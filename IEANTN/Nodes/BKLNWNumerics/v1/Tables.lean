...
this node's `Conclusions.lean`.

Copied from `BKLNW.v1` so the numerical conclusions can live on a computation node
without editing the paper node's statements.
