...
-- Dusart's Proposition 5.4 is a *hypothesis* here, not an import.
--
-- In PNT+ this development calls `Dusart.proposition_5_4` directly. That is unproved there
-- (`Dusart.proposition_5_4c` is a hole), so PNT+'s `L_not_HA_of_ge` depends on `sorryAx` and
-- could never pass Comparator. The node's challenge
-- already states the result conditionally on Dusart, so threading it as a hypothesis is both
-- what the challenge requires and what makes the proof honest about what it rests on.
section Dusart

variable (hd : Dusart2018.v1.proposition_5_4)
...
theorem prod_q_ge {n : ℕ} (hn : n ≥ X₀ ^ 2) :
    ∏ i, (1 + (1 : ℝ) / (exists_q_primes hd hn).choose i) ≤
  ...
  intro i _
  suffices h : (1 : ℝ) / (exists_q_primes hd hn).choose i ≤
      (1 + 1 / (log √(n : ℝ)) ^ 3) ^ ((3 : ℝ) - (i : ℕ)) / n from (by linarith)
  have := (exists_q_primes hd hn).choose_spec.2.2.1 i
...
theorem prod_p_ge {n : ℕ} (hn : n ≥ X₀ ^ 2) :
    ∏ i, (1 + (1 : ℝ) /
        ((exists_p_primes hd hn).choose i * ((exists_p_primes hd hn).choose i + 1))) ≥
      ∏ i : Fin 3,
        (1 + 1 / ((1 + 1 / (log √(n : ℝ)) ^ 3) ^ (2 * (i : ℕ) + 2 : ℝ) * (n + √n))) := by
  refine Finset.prod_le_prod (fun i _ => by positivity [hlog hn]) fun i _ => ?_
  set p := (exists_p_primes hd hn).choose
  have h₀ (i) : √n < p i := by
    have : p 0 ≤ p i := by
      apply (exists_p_primes hd hn).choose_spec.2.1.monotone
      simp
    grw [← this]
    exact (exists_p_primes hd hn).choose_spec.2.2.2
  gcongr 1 + 1 / ?_
  · have := ((exists_p_primes hd hn).choose_spec.1 i).pos
    positivity
  have : p i ≤ √n * (1 + 1 / log √n ^ 3) ^ (i + 1 : ℝ) :=
    (exists_p_primes hd hn).choose_spec.2.2.1 i
...
theorem pq_ratio_ge {n : ℕ} (hn : n ≥ X₀ ^ 2) :
    1 - ((4 : ℝ) * ∏ i, ((exists_p_primes hd hn).choose i : ℝ))
    / ∏ i, ((exists_q_primes hd hn).choose i : ℝ) ≥
  ...
  · exact (exists_p_primes hd hn).choose_spec.2.2.1 _
  · exact fun _ _ => by positivity [hε_pos hn]
  · exact (exists_q_primes hd hn).choose_spec.2.2.1 _
...
noncomputable def Criterion.mk' {n : ℕ} (hn : n ≥ X₀ ^ 2) : Criterion where
  n := n
  p := (exists_p_primes hd hn).choose
  q := (exists_q_primes hd hn).choose
  hn := le_trans (by decide : 1 ≤ 89693 ^ 2) hn
  hp := (exists_p_primes hd hn).choose_spec.1
  hp_mono := (exists_p_primes hd hn).choose_spec.2.1
  hq := (exists_q_primes hd hn).choose_spec.1
  hq_mono := (exists_q_primes hd hn).choose_spec.2.1
  h_ord_1 := (exists_p_primes hd hn).choose_spec.2.2.2
  h_ord_2 := by
    have hn_pos : (0 : ℝ) < n := by positivity
    have hp' : ((exists_p_primes hd hn).choose 2 : ℝ) ≤ √n * (1 + 1 / (log √n) ^ 3) ^ 3 := by
      convert (exists_p_primes hd hn).choose_spec.2.2.1 2 using 2; norm_cast
    have hq' : n * (1 + 1 / (log √n) ^ 3) ^ (-3 : ℝ) ≤ (exists_q_primes hd hn).choose 0 := by
      convert (exists_q_primes hd hn).choose_spec.2.2.1 0 using 2
      norm_num
  ...
  h_ord_3 := (exists_q_primes hd hn).choose_spec.2.2.2
  h_crit := by
    have hn₀ : 0 ≤ Real.log √n := by
      grw [hn]
      simp [log_nonneg]
    have h₁ : 1 - (4 : ℝ) *
        (∏ i, (exists_p_primes hd hn).choose i : ℝ) / ∏ i, ((exists_q_primes hd hn).choose i : ℝ) ≥
        1 - 4 * (1 + 0.000675) ^ 12 * ((1 / 89693) * (1 / n)) := by
      grw [pq_ratio_ge hd hn, inv_cube_log_sqrt_le hn, ← inv_n_pow_3_div_2_le hn]
      simp [field]
    ...
    grw [prod_q_ge hd hn, prod_p_ge hd hn, h₁]
...
theorem L_not_HA_of_ge (n : ℕ) (hn : n ≥ 89693 ^ 2) : ¬HighlyAbundant (L n) :=
  (Criterion.mk' hd hn).not_highlyAbundant
