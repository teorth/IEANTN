...
- Of the family's other versions, only CH2.v2 is an edge. Proposition 2.4 (CH2.v2) is imported and used as stated: the solution applies it to the explicit majorant and minorant through a two-line adapter, the statements agreeing definitionally. CH2.v3 is NOT an edge, and cannot be as stated: it asserts that an extremal approximant EXISTS, while sections 5-6 evaluate the main term by shifting a contour through the specific Graham-Vaaler function's meromorphic continuation -- the (pi/T) coth(pi(1-sigma)/T) main term and the zero terms come from its explicit residues -- and an existentially given function has no continuation to shift through. So the solution constructs the function itself (Approximants.lean, the same construction CH2.v3's solution verifies). The contour shift is PNT+'s own section 5, not CH2.v4, which is inactive for that reason.
